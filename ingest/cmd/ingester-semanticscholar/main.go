// ingester-semanticscholar: Semantic Scholar Graph API literature ingester.
package main
...
import (
	...
	"github.com/evidencelens/evidencelens/ingest/internal/ingesters/semanticscholar"
...
func main() {
	...
	defer cancel()
	logger := ingestcommon.MustNewLogger("ingester-semanticscholar")
	shutdown, _ := otel.Init(ctx, "ingester-semanticscholar")
	...
	ing := semanticscholar.New(semanticscholar.Config{
		MaxPerRun: ingestcommon.GetEnvInt("S2_MAX_PER_RUN", 5000),
		Query:     ingestcommon.GetEnv("S2_QUERY", ""),
	}, logger, wm, arch, pub)
	runner := &ingestcommon.Runner{Source: "semanticscholar", Logger: logger, Run: ing.Run}
